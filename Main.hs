...
--------------------------------------------------------------------------------------------------------
eval1' t= case (parsear t) of
                []-> print "Error de parseo, revise el termino ingresado"
                t'-> let a=runErrorMT(evalTerm1 (conversion (fst (head t'))))
                      in a >>= (\i -> case i of
                                    JustE x -> displayImage x--writeImage "/home/nico/Desktop/output.png" x
                                    EM s -> print s)

eval2' t= case (parsear t) of
                []-> print "Error de parseo, revise el termino ingresado"
                t'-> let a=runErrorMT(evalTerm2 (conversion (fst (head t'))))
                      in a >>= (\i -> case i of
                                    JustE x -> displayImage x--writeImage "/home/nico/Desktop/output.png" x
                                    EM s -> print s)

eval3' t= case (parsear t) of
                []-> print "Error de parseo, revise el termino ingresado"
                t'-> let a=runErrorMT(evalTerm3 (conversion (fst (head t'))))
                      in a >>= (\i -> case i of
                                    JustE x -> displayImage x--writeImage "/home/nico/Desktop/output.png" x
                                    EM s -> print s)
...
--Declaraciones asociadas a la ejecucion del programa compilado
-----------------------------------------------------------------------------------------
